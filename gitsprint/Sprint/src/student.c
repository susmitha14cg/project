...
int studSignUp(int __id)
{
	...
	printf("\n\tDepartment: ");
	scanf("%s", ss->dept);
	printf("\n\tEnter the status of placement(YES/NO): ");
	scanf("%s",ss->placed);
	ss->regno = __id;
	sl->regno = __id;
	printf("\n\tUser Name: ");
...
int viewAlumniDetails(int uni_id)
{
	FILE *fp;
...
void askConnect(STUDCONN *sc,int _id )
{
	int i=0,ch;
	FILE *fp;
	...
	do
	{
		printf("Enter id of alumni");
		scanf("%d",&sc->cAlumniId[i]);
		printf("\n\tDo you want to continue(0/1):");
		scanf("%d",&ch);
		i++;
	}
	while(ch>0);
	fwrite(sc,sizeof(STUDCONN),1,fp);
